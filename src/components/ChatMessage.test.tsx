...
describe('ChatMessage', () => {
  ...
  test('user message is right-aligned', () => {
    const { container } = render(
      <ChatMessage message={baseMsg({ role: 'user', content: 'Hi there' })} />,
    )
  ...
  test('assistant message is left-aligned', () => {
    const { container } = render(
      <ChatMessage
        message={baseMsg({ role: 'assistant', content: 'Hello!' })}
      />,
    )
    const wrapper = container.firstChild as HTMLElement
  ...
  test('user message bubble has primary background', () => {
    const { container } = render(
      <ChatMessage message={baseMsg({ role: 'user' })} />,
    )
  ...
  test('assistant message bubble has muted background', () => {
    const { container } = render(
      <ChatMessage message={baseMsg({ role: 'assistant' })} />,
    )
    const bubble = (container.firstElementChild as HTMLElement)
      .firstElementChild as HTMLElement
  ...
  test('user message renders plain text (not markdown)', () => {
    render(
      <ChatMessage
        message={baseMsg({ role: 'user', content: '**bold** text' })}
      />,
    )
  ...
  test('assistant message renders bold markdown', () => {
    render(
      <ChatMessage
        message={baseMsg({ role: 'assistant', content: '**Available Units**' })}
      />,
    )
