...
vi.mock('firebase/auth', () => ({
  onAuthStateChanged: vi.fn((auth, cb) => {
    ...
    cb({ uid: 'test-uid', getIdToken: async () => 'fake-token' })
    return () => {}
  }),
...
const msg = (role: 'user' | 'assistant', content: string, id = crypto.randomUUID()) => ({
  id,
  role,
  content,
  timestamp: Date.now(),
...
async function renderWithMessages(messages: ReturnType<typeof msg>[]) {
  await act(async () => { render(<ChatWindow />) })
...
describe('ChatWindow — US-005 lead capture', () => {
  ...
  test('"Contact me" button hidden after leadCaptured=true', async () => {
    await renderWithMessages([msg('user', 'hi'), msg('assistant', 'hello')])
    await act(async () => { useChatStore.getState().setLeadCaptured(true) })
    expect(screen.queryByRole('button', { name: /contact me/i })).toBeNull()
  })

  test('Lead form opens on "Contact me" click', async () => {
    await renderWithMessages([msg('user', 'hi'), msg('assistant', 'hello')])
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: /contact me/i })) })
  ...
  test('Lead form: name and phone are required fields', async () => {
    await renderWithMessages([msg('user', 'hi'), msg('assistant', 'hello')])
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: /contact me/i })) })
    const nameInput = screen.getByPlaceholderText(/full name/i) as HTMLInputElement
    const phoneInput = screen.getByPlaceholderText(/phone number/i) as HTMLInputElement
  ...
  test('AC-U: confirmation message shown after lead form submit', async () => {
    const { upsertLead } = await import('../lib/firestore')
    vi.mocked(upsertLead).mockResolvedValue('lead-id-123')

    await renderWithMessages([msg('user', 'hi'), msg('assistant', 'hello')])
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: /contact me/i })) })

    await act(async () => {
      fireEvent.change(screen.getByPlaceholderText(/full name/i), { target: { value: 'Somchai Jaidee' } })
      fireEvent.change(screen.getByPlaceholderText(/phone number/i), { target: { value: '0812345678' } })
    })
    await act(async () => { fireEvent.click(screen.getByRole('button', { name: /submit/i })) })

    expect(screen.getByText(/thank you somchai jaidee, a leasing specialist will contact you within 1 business day/i)).toBeDefined()
  })
})

describe('ChatWindow — streaming indicator', () => {
  test('3-dot bounce shown when isStreaming=true and last message is empty', async () => {
    await renderWithMessages([msg('user', 'hi'), msg('assistant', '', 'streaming-msg')])
    await act(async () => { useChatStore.getState().setStreaming(true) })
